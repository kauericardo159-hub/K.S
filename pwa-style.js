(function() {
    ...
    estilos.textContent = `
        ...
        .splash-logo {
            ...
            height: 130px;
            object-fit: contain;
            filter: drop-shadow(0 8px 30px rgba(0, 229, 255, 0.2));
            opacity: 0;
    ...
    if (estaNoApp) {
        // --- MODO APLICATIVO INSTALADO (SPLASH SCREEN) ---
        ...
        // --- MODO NAVEGADOR (PROMPT DE INSTALAÇÃO CUSTOMIZADO) ---
        const checarBannerRecusadoRecente = () => {
            const dataRecusa = localStorage.getItem('pwa-banner-recusado-data');
            if (!dataRecusa) return false;
            
            // Pergunta novamente após 7 dias da recusa
            const seteDiasEmMs = 7 * 24 * 60 * 60 * 1000;
            return (Date.now() - parseInt(dataRecusa, 10)) < seteDiasEmMs;
        };

        if (checarBannerRecusadoRecente()) return;

        let capturaInstalacao = null;

        const exibirBanner = (instrucaoIOS = false) => {
            ...
            banner.innerHTML = `
                ...
                <div class="pwa-info-texto">
                    ...
                    <span class="pwa-txt-desc">${instrucaoIOS ? 'Toque em Compartilhar e "Adicionar à Tela de Início"' : 'Acesse direto da sua tela inicial'}</span>
                </div>
                ${!instrucaoIOS ? '<button class="btn-instalar-pwa" id="gatilho-instalar-pwa">Instalar</button>' : ''}
            ...
            const btnInstalar = document.getElementById('gatilho-instalar-pwa');
            if (btnInstalar) {
            ...
            document.getElementById('fechar-pwa-banner').addEventListener('click', () => {
                banner.classList.remove('mostrar');
                localStorage.setItem('pwa-banner-recusado-data', Date.now().toString());
                setTimeout(() => banner.remove(), 500);
            });
        };

        // Captura do evento nativo em navegadores suportados (Android / Chrome / Edge)
        ...
        // Suporte para iOS/Safari (exibe instrução manual simples)
